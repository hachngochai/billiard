...
function () {
  ...
  _createClass(Table, [{
    ...
    value: function buildBounds() {
      ...
      var h = VIEW_H;
      this.bounds = [// Top
      Bodies.rectangle(VIEW_W * 0.5, h * -0.5, hw, h, boundsOptions), // Bottom
      Bodies.rectangle(VIEW_W * 0.5, VIEW_H + h * 0.5, hw, h, boundsOptions), // Left
      Bodies.rectangle(vw * -0.5, VIEW_H * 0.5, vw, h, boundsOptions), // Left
    ...
    value: function buildWall() {
      ...
      Bodies.fromVertices(rel(TABLE_W / 4), rel(TABLE_H + WALL_RAD), vertices.bottom, wallOptions), // Bottom Right
      Bodies.fromVertices(rel(TABLE_W / 4 + TABLE_W / 2), rel(TABLE_H + WALL_RAD), vertices.bottom, wallOptions), // Top Left
      Bodies.fromVertices(rel(TABLE_W / 4), rel(0 - WALL_RAD), vertices.top, wallOptions), // Top Right
      Bodies.fromVertices(rel(TABLE_W / 4 + TABLE_W / 2), rel(0 - WALL_RAD), vertices.top, wallOptions), // Left
      Bodies.fromVertices(rel(0 - WALL_RAD), rel(TABLE_H / 2), vertices.left, wallOptions), // Right
      Bodies.fromVertices(rel(TABLE_W + WALL_RAD), rel(TABLE_H / 2), vertices.right, wallOptions), // TL horizontal
      Bodies.rectangle(horLX, horTY, horizontalBlock.width, horizontalBlock.height, wallOptions), // TR horizontal
      Bodies.rectangle(horRX, horTY, horizontalBlock.width, horizontalBlock.height, wallOptions), // BL horizontal
      Bodies.rectangle(horLX, horBY, horizontalBlock.width, horizontalBlock.height, wallOptions), // BR horizontal
      Bodies.rectangle(horRX, horBY, horizontalBlock.width, horizontalBlock.height, wallOptions), // TL vertical
      Bodies.rectangle(verLX, verTY, verticalBlock.width, verticalBlock.height, wallOptions), // TR vertical
      Bodies.rectangle(verRX, verTY, verticalBlock.width, verticalBlock.height, wallOptions), // BL vertical
      Bodies.rectangle(verLX, verBY, verticalBlock.width, verticalBlock.height, wallOptions), // BR vertical
      Bodies.rectangle(verRX, verBY, verticalBlock.width, verticalBlock.height, wallOptions), // B middle
      Bodies.rectangle(rel(TABLE_W / 2), horBY, middleBlock.width, middleBlock.height, wallOptions), // T middle
...
function () {
  ...
  _createClass(Canvas, [{
    ...
    value: function drawIndicator(_ref12) {
      ...
      this.angleSin = Math.sin(this.angle); // coordinates for starting power just off the cueball

      var lineMinX = this.cueX + BALL_DI * 1.2 * this.angleCos;
      var lineMinY = this.cueY + BALL_DI * 1.2 * this.angleSin; // coordinates for showing power

      var lineMaxX = lineMinX + maxDistance * this.angleCos;
      var lineMaxY = lineMinY + maxDistance * this.angleSin; // coordinates for calculating power

      var newX = lineMinX + power * maxDistance * this.angleCos;
      var newY = lineMinY + power * maxDistance * this.angleSin; // setting the force relative to power

      this.forceX = (newX - lineMinX) / maxDistance * 0.02;
      this.forceY = (newY - lineMinY) / maxDistance * 0.02;
      this.context.lineCap = 'round'; // max power
      ...
      this.context.stroke();
      this.context.closePath(); // power level
    ...
    value: function drawBall(ball) {
      var x = ball.body.position.x,
          ...
          a = ball.body.angle;
      this.context.translate(x, y);
      this.context.rotate(a); // offset from center
  ...
  _createClass(Game, [{
    ...
    value: function handleTurnEnd() {
      ...
      var isCue = pocketed.filter(function (b) {
        ...
        return b.eight;
      }).length > 0; // determining valid first contact
      ...
      if (pocketed.length > 0) {
        var stripes = pocketed.filter(function (b) {
          ...
          return b.solids;
            ...
            hasSolids = solids.length > 0; // assigning players
        ...
        if (isEight) {
          this.messageEight();
          winner = this.currentPlayer.onEight ? this.currentPlayer : this.otherPlayer; // handling cueball
        } else if (isCue) {
          this.messageScratch();
          this.switchTurns(); // handling invalid contact
        } else if (!validFirstContact) {
          this.messageInvalidContact();
          this.switchTurns(); // handling the wrong ball
        ...
        this.messageScratch();
        this.switchTurns(); // switching turns on nothing going in
      } else {
        this.switchTurns();
      } // ending the turn
